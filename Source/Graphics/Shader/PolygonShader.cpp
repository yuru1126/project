...
PolygonShader::PolygonShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// 入力レイアウト
		...
		create_vs_from_cso(device, "Shader/PolygonVS.cso",
			vertexShader.ReleaseAndGetAddressOf(), inputLayout.ReleaseAndGetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
	}

	// ピクセルシェーダー
	{
		create_ps_from_cso(device, "Shader/PolygonPS.cso", pixelShader.ReleaseAndGetAddressOf());
	}

	// ジオメトリシェーダー
	{
		create_gs_from_cso(device, "Shader/PolygonGS.cso", geometryShader.ReleaseAndGetAddressOf());
	}

	// ハルシェーダー
	{
		create_hs_from_cso(device, "Shader/PolygonHS.cso", hullShader.ReleaseAndGetAddressOf());
	}

	// ドメインシェーダー
	{
		create_ds_from_cso(device, "Shader/PolygonDS.cso", domainShader.ReleaseAndGetAddressOf());
	}
	...
	// 定数バッファ
	{
		...
		//ポリゴンシェーダー用バッファ
		desc.ByteWidth = sizeof(PolygonData);

		hr = device->CreateBuffer(&desc, 0, polygonConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
...
// 描画開始
void PolygonShader::Begin(const RenderContext& rc)
{
...
// 必要なバッファの設定
void PolygonShader::SetBuffers(const RenderContext& rc, const std::vector<Model::Node>& nodes, const ModelResource::Mesh& mesh)
{
	//ポリゴンシェーダー用バッファ更新
	PolygonData polygon_data = rc.polygon_data;
	rc.deviceContext->UpdateSubresource(polygonConstantBuffer.Get(), 0, 0, &polygon_data, 0, 0);
...
// サブセット単位で描画
void PolygonShader::DrawSubset(const RenderContext& rc, const ModelResource::Subset& subset)
{
	CbSubset cbSubset;
...
// 描画終了
void PolygonShader::End(const RenderContext& rc)
{
