...
beforeSprite::beforeSprite()
	: beforeSprite(nullptr)
{
}

// コンストラクタ
beforeSprite::beforeSprite(const char* filename)
{
	ID3D11Device* device = Graphics::Instance().GetDevice();
...
beforeSprite::beforeSprite(Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> shader_resource_view)
{
	ID3D11Device* device = Graphics::Instance().GetDevice();
	...
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PostEffectVS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\PostEffectPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// 定数バッファ
	{
		...
		HRESULT hr = device->CreateBuffer(&desc, 0, resolutionConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
...
// 描画実行
void beforeSprite::Render(ID3D11DeviceContext *immediate_context,
	float dx, float dy,
	float dw, float dh,
	float sx, float sy,
	float sw, float sh,
	...
	float r, float g, float b, float a) const
{
...

	{
		...
		{
			Resolution data{};
			data.divide.x = setDivide;
			immediate_context->UpdateSubresource(resolutionConstantBuffer.Get(),
				0, 0, &data, 0, 0);
			immediate_context->PSSetConstantBuffers(0, 1, resolutionConstantBuffer.GetAddressOf());
		}
...
void beforeSprite::DrawDebugGUI()
{
	ImGui::SetNextWindowPos(ImVec2(10, 10), ImGuiCond_FirstUseEver);
	ImGui::SetNextWindowSize(ImVec2(300, 300), ImGuiCond_FirstUseEver);
	if (ImGui::Begin("Sprite", nullptr, ImGuiWindowFlags_None))
	{
		ImGui::SliderFloat("divide", &setDivide, 1.0, 5.0);
	}
	ImGui::End();
}