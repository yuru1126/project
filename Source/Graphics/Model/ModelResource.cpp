...
void ModelResource::BuildModel(ID3D11Device* device, const char* dirname)
{
	for (Material& material : materials)
	{
		...
		// 金属/滑らかさマップテクスチャ読み込み
		// PBRテクスチャは各エンジンごとに違ったりする。
		// Unityは以下のパターン
		// r : 金属質
		// gb : 空き
		// a : 滑らかさ
		// UnrealEngineの場合は別である
		LoadTexture(device, filename, "_MS", true, material.metallic_smoothness_map.GetAddressOf(), 0x00FFFF00);

		//#2022 Metallic/Smoothness別読み
		{
			LoadTexture(device, filename, "_M", true, material.metallic_map.GetAddressOf(), 0xFFFFFFFF);
			LoadTexture(device, filename, "_R", true, material.roughness_map.GetAddressOf(), 0xFF404040);
		}

		// 環境光遮蔽マップテクスチャ読み込み
		LoadTexture(device, filename, "_AO", true, material.ambient_occlusion_map.GetAddressOf(), 0xFFFFFFFF);
